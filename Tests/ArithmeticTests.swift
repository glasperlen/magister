...
final class ArithmeticTests: XCTestCase {
    ...
    func testDistance() {
        ...
        board[2, 0] = .init(order: .second, bead: .init(.init(left: 1)))
    ...
    func testLess() {
        board[0, 0] = .init(order: .first, bead: .init(.init(right: 1)))
        board[1, 0] = .init(order: .second, bead: .init(.init(left: 0)))
        XCTAssertEqual(0, board[.first].score)
        XCTAssertEqual(0, board[.second].score)
    }
    ...
    func testEqual() {
        board[0, 0] = .init(order: .first, bead: .init(.init()))
        board[1, 0] = .init(order: .second, bead: .init(.init()))
        XCTAssertEqual(0, board[.first].score)
        XCTAssertEqual(0, board[.second].score)
    }
}
