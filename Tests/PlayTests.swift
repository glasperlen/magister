...
final class PlayTests: XCTestCase {
    ...
    func testFinish() {
        (0 ..< 3).forEach { x in
            (0 ..< 3).forEach { y in
                ...
                match[.init(x, y)] = .init(player: .opponent, bead: .init(), point: .init(x, y))
            }
        }
        match.turn = .opponent
        match.robot()
        XCTAssertEqual(.opponent, match.turn)
        XCTAssertNotNil(match.result)
    }
    
    func testFinishWin() {
        (0 ..< 3).map { .init($0, 0) }.forEach {
            match[$0] = .init(player: .opponent, bead: .init(), point: $0)
        }
    ...
    func testFinishLoose() {
        ...
        match[.init(0, 1)] = .init(player: .user, bead: .init(), point: .init(0, 1))
        match[.init(1, 1)] = .init(player: .opponent, bead: .init(), point: .init(1, 1))
        match.play(.init(), .init(2, 1))
